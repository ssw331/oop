...
class world : public haha, public what, virtual public Point  //虚基类--为了防止间接继承同一基类时发生成员冗余而提供的机制（C++）
	//需要在此虚基类的所有直接派生类中均用virtual关键词来声明，否则不会释放重复资源
	//所有与虚基类有关的派生类的构造函数都需要对虚基类进行初始化
	//虚基类的构造函数调用先于非虚基类
...
public:
	world():haha(), what(),str(NULL),test(-1)//派生类自身的构造函数    
		//格式：派生类名（形参列表）: 基类名1（基类初始化实参），基类名2（实参）...<这里是类的构造函数的调用声明，不是对象的，编译时会根据这里的参数量来调用对应的基类构造函数>,派生类成员的初始化列表<这里面可以有对象,但不能有基类的成员> 
		// {函数体}
		//派生类构造函数的执行顺序：基类构造函数<按照声明次序调用>-》派生类初始化列表<按照声明次序调用>-》派生类构造列表的函数体
		//类似这里实例化的构造函数中的无参数初始化（即有默认构造函数）的基类/子对象，可以把它的初始化隐式化[派生类构造函数会默认调用基类/子对象默认构造函数]
	...
	void helloworld()//派生类可以定义和基类成员相同的函数，会覆盖基类的同名函数，同时不会改写基类的对应函数<相当于重载+下一行的陈述>
		...
		//1. 重载多态
		//2. 强制多态 即类型转换
		//3. 类型参数化多态：模板template
		// 以上为静态多态
		//*4*. 包含多态：C++中利用 虚函数 实现， 包含虚函数的类被称为多态类，只有运行时可以确定多态
		// 
		// 联编（绑定）：
		//	即将模块或函数合并在一起生成可执行代码的处理过程。
		// 会对每个模块/函数分配内存。
		// 
		// 其实就是将代码以块的方式进行编译的过程，例如定义一个对象，会生成对象的类的相关函数和基类
		// 
		// 
		// 静态联编：
		//	在编译阶段就将函数实现和函数调用绑定（联编），在编译阶段就必须了解所有函数火模块所需的信息
		// 对函数的选择基于指向对象的指针（或引用）的类型
		// ---》即如果你是基类类型的对派生类对象的引用/指针，那么你用这个引用/指针来调用的成员函数是基类的成员而不是派生类的。
		// 
		// C中所有联编都是静态，C++一般也是
		// 
		// 动态联编：
		//	即程序运行时进行绑定。
		// 
		// 由于静态联编的存在，如果需要基类类型的引用或指针调用派生类的成员函数，
		// 那么需要将基类的该成员函数变为虚函数。这样就能实现动态绑定
		// 虚函数：在函数前加上virtual关键字
		// 虚函数表会被派生类继承，而且在派生类中重载基类的函数时，编译器会隐式地将派生类中的重载函数转换为虚函数
		//
	{
		return;
	}
...
protected://对于继承和派生，有时需要派生类访问不被用户访问的成员，此时需要将其定义在protected下
	...
	//二义性问题：
	/*
	* 在继承和派生的过程中会出现同名的成员，那么在调用的时候就会出现二义性
	* 因此需要用成员名限定
	* 格式; 对象名.基类名::成员名
	*		对象指针名->基类名::成员名
	* 
	* 名字支配原则：内层同名成员在内层会覆盖外层同名（即此时不需要成员名限定即可调用内层同名成员）
	*				如果要调用被覆盖的，可以用成员名限定
	*				对于派生，基类作用域在派生类作用域的外层
	*/
};

class O : public Point;//这个是错误的，派生类的前向声明只要声明派生类名即可
class OP;
class complex
{
public:
...
// .  .* （指向成员的指针的运算符）   ::   ?:   sizeof
// 
// 可以定义成员的指针，而调用指向成员的指针需要用.*
// 
// 2. 不能改变优先级、结合性和运算对象数
//// 3. 不可使用默认参数
// 4. 重载运算符必须有一个类对象/对象的引用作为参数
// 5. 一般如果运算结果为左值则返回引用，右值则返回对象
// 6. 重载前后功能一致
// 7. 运算符重载作为类成员函数时，所用参数列表会少一个（因为this作为隐式参数载入）
//
complex operator+(const complex& a, const complex& b)
{
	complex sum;
	sum.imag = a.imag + b.imag;
	sum.real = a.real + a.real;
	return sum;//别python写多了就忘记C++的函数在返回类型不为void的时候一定要return啊混蛋
}
